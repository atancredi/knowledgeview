					...
					if (rawText.startsWith('---')) {
						...
						if (parsed.attributes.tags) {
							tags = Array.isArray(parsed.attributes.tags)
								...
								: parsed.attributes.tags.split(',').map((t) => t.trim());
						}
					}
			...
			<div style={{ display: 'flex', height: '100vh', alignItems: 'center', justifyContent: 'center', backgroundColor: 'var(--bg-secondary)' }}>
				<label
					...
					style={{
						...
						backgroundColor: isBtnHovered ? 'var(--zinc-800)' : 'var(--interactive-active)',
						color: 'var(--text-inverse)',
						borderRadius: 'var(--radius-md)',
						cursor: 'pointer',
						fontWeight: 500,
				...
				>
					...
					<input type="file" webkitdirectory="true" directory="true" multiple onChange={handleFolderSelect} style={{ display: 'none' }} />
				</label>
			</div>
		);
	}
